	...
	public function admin_page() {
		...
		switch($_POST['action']) {
			...
			case 'refresh_category_tree':
				...
				if($bUpdateResult != false) {
					...
					$msg = array('message' => 'Fehler: Konnte Widget-Optionen nicht aktualisieren. Entweder die Kategoriebäume haben sich nicht verändert (überflüssiges Refresh), oder es ist ein unbekannter Fehler beim Schreiben der Optionen in die Datenbank aufgetreten.');
				}
			...
			case 'update_settings':
				...
				if($bUpdateResult != false) {
					...
					$msg = array(
						'type' => 'error',
						'message' => 'Fehler: Konnte Widget-Optionen nicht aktualisieren. Entweder die Daten haben sich nicht verändert (überflüssiges Refresh), oder es ist ein unbekannter Fehler beim Schreiben der Optionen in die Datenbank aufgetreten.'
					);
				}
				break;
		}
		...
		<div class="wrap">
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo $_GET['page']; ?>" id="form-category-tree-refresh">
				...
				<p><button type="submit" class="button-submit button-secondary" title="">Kategorie-Bäume aktualisieren</button></p>
				
				<p><em><strong>Hinweis:</strong> Die Neugenerierung der Kategorie-Bäume kann etwas dauern, daher <strong>bitte die Seite NICHT neu laden</strong>, falls sich der Seitenaufbau etwas hinauszögert!</em></p>
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo $_GET['page']; ?>" id="form-main">
				...
				<p>&nbsp; <button type="submit" class="button button-submit button-primary">Änderungen speichern</button></p>
				...
				<table class="widefat" style="width: 70%">
					<thead>
						<tr>
							<th class="manage-column column-id" id="id" scope="col">ID</th>
							<th class="manage-column column-category" scope="col">Basiskategorie</th>
							<th class="manage-column column-depth" id="depth" scope="col">Baumtiefe (Ebenen)</th>
							<th class="manage-column column-date" id="date" scope="col">Nächste Aktualisierung</th>
							<th class="manage-column column-date" id="date" scope="col">Cache-Timeout (in Stunden)</th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							...
							<th class="manage-column column-depth" id="depth" scope="col">Baumtiefe (Ebenen)</th>
							<th class="manage-column column-date" id="date" scope="col">Nächste Aktualisierung</th>
							<th class="manage-column column-cache-timeout" id="cache-timeout" scope="col">Cache-Timeout (in Stunden)</th>
						</tr>
					</tfoot>
					<tbody>
					<?php 
					if(!empty($arrWidgets) ) {
						...
						<tr class="<?php echo ($iRowCount % 2) ? 'alt' : ''; ?>">
							...
							<td class="date column-date"><?php echo date('d.m.Y H:i:s', $widgetData['category_tree_expiration']); ?></td>
...
class _ui_MultipleCategoryFilterWidget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			...
			<small>After how many <strong>hours</strong> shall the selected category tree be refreshed? Will be automatically be set to 48 hours if no value is entered.</small>
		</p>
